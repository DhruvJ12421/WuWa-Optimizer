...
export function WeaponInventory({ owned, characters = [], builds = [], refresh }: { owned: OwnedWeapon[]; characters?: OwnedCharacter[]; builds?: Build[]; refresh: () => Promise<void> }) {
  ...
  const deferred = useDeferredValue(query.toLowerCase())
  const visible = useMemo(() => owned.map((item) => ({ item, catalog: weaponCatalog.find((entry) => entry.id === item.catalogId) })).filter(({ catalog }) => catalog && (type === 'all' || catalog.type === type) && (rarity === 'all' || catalog.rarity === rarity) && `${catalog.name} ${catalog.type} ${catalog.secondaryStat}`.toLowerCase().includes(deferred)).sort((left, right) => (right.catalog?.rarity ?? 0) - (left.catalog?.rarity ?? 0) || (left.catalog?.name ?? '').localeCompare(right.catalog?.name ?? '')), [deferred, owned, rarity, type])
  const available = weaponCatalog.filter((entry) => (pickerType === 'all' || entry.type === pickerType) && (pickerRarity === 'all' || entry.rarity === pickerRarity) && `${entry.name} ${entry.type} ${entry.secondaryStat}`.toLowerCase().includes(pickerQuery.toLowerCase())).sort((left, right) => right.rarity - left.rarity || left.name.localeCompare(right.name))
