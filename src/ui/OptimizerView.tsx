...
import { characterFormulaSheets, createBuildCalculationContext, resolveRuntimeBuild } from '../domain/calculation'
...
export function OptimizerView({ echoes, builds, characters, ownedWeapons, refresh, openScanner }: { echoes: Echo[]; builds: Build[]; characters: OwnedCharacter[]; ownedWeapons: OwnedWeapon[]; refresh: () => Promise<void>; openScanner: () => void }) {
  ...
  return <>
    <PageHeader eyebrow="Formula-driven search" title="Echo optimizer" description="Optimize a Nanoka-derived ability result in fixed build context. Completed searches are exact; capped searches are labeled best found." actions={running ? <button className="danger" onClick={cancel}>Cancel search</button> : <button className="primary" onClick={run}><Icon name="optimize"/>Run optimization</button>} />
    <div className="optimizer-layout"><Panel className="optimizer-config"><div className="section-heading"><div><span className="eyebrow">Search target</span><h2>Configuration</h2></div></div><label>Character build<select value={build?.id} onChange={(event) => setBuildId(event.target.value)}>{builds.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label>Formula target<select value={attack?.id} onChange={(event) => setAttackId(event.target.value)}>{resonator?.attacks.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label>Objective<select value={objective} onChange={(event) => setObjective(event.target.value as OptimizerObjective)}><option value="expected">Average damage</option><option value="normal">Non-CRIT damage</option><option value="critical">CRIT damage</option><option value="atk">ATK</option><option value="hp">HP</option><option value="critRate">Crit. Rate</option><option value="critDamage">Crit. DMG</option><option value="energyRegen">Energy Regen</option></select></label><label>Required 5-piece Sonata<select value={sonata} onChange={(event) => setSonata(event.target.value)}><option value="">Any Sonata</option>{sonataNames.map((name) => <option key={name}>{name}</option>)}</select></label><div className="field-row"><label>Enemy level<input type="number" min="1" max="200" value={enemyLevel} onChange={(event) => setEnemyLevel(Number(event.target.value))}/></label><label>Resistance %<input type="number" min="-100" max="100" value={enemyResistance} onChange={(event) => setEnemyResistance(Number(event.target.value))}/></label></div><label>Minimum Crit. Rate <span>{minCrit}%</span><input type="range" min="0" max="100" value={minCrit} onChange={(event) => setMinCrit(Number(event.target.value))}/></label><label>Minimum Energy Regen <span>{minEnergy || 100}%</span><input type="range" min="0" max="250" value={minEnergy} onChange={(event) => setMinEnergy(Number(event.target.value))}/></label><div className="config-note"><strong>{echoes.filter((echo) => !echo.excluded && (!echo.equippedBy || echo.equippedBy === build?.id)).length}</strong><span>available candidates</span><small>All legal candidates are considered. Locked pieces are mandatory. Capped searches are explicitly marked best found.</small></div>{error && <div className="notice error">{error}</div>}{message && <div className="notice success">{message}</div>}</Panel>
