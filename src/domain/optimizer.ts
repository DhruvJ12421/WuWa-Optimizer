...
export function optimizeBuilds(request: OptimizerRequest, maxEvaluations = 300_000): OptimizerResult[] {
  ...
  const usable = request.echoes.filter((echo) => {
    if (echo.excluded || echo.equippedBy || seenIds.has(echo.id)) return false
    seenIds.add(echo.id)
    return true
  })
  ...
  const candidates = [1, 3, 4].flatMap((cost) => ranked.filter((echo) => echo.cost === cost).slice(0, 28))
  ...
  function visit(index: number, selected: Echo[], cost: number) {
    ...
    if (selected.length === 5) {
      ...
      if (!meetsMinimums(stats, request.minimumStats)) return
      ...
      const result = { ...partial, score: resultScore(partial, request.objective) }
      results.push(result)
