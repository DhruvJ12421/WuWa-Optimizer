export type StatKey = 'hp' | 'hpPercent' | 'atk' | 'atkPercent' | 'def' | 'defPercent' | 'critRate' | 'critDamage' | 'energyRegen' | 'basicDamage' | 'heavyDamage' | 'skillDamage' | 'liberationDamage' | 'spectroDamage' | 'fusionDamage' | 'glacioDamage' | 'healingBonus'
...
export interface AccountDocument { schemaVersion: 1; gameDataVersion: string; exportedAt: string; echoes: Echo[]; builds: Build[]; teams: Team[]; settings: AppSettings }
...
export type AppView = 'dashboard' | 'scanner' | 'inventory' | 'builds' | 'teams' | 'optimizer'
